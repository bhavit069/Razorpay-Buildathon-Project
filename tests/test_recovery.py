...
def test_no_discretionary_rung_is_ever_added_at_a_loss():
    ...
    that is what escalating means - and those are flagged rather than averaged
    in with the rest."""
...
def test_optimism_is_bounded_by_the_declared_direction(ledger):
    ...
    cases, and turning LTV on makes it do just that - 1144 cases
...
def test_every_case_the_system_did_not_uphold_is_actioned(ledger):
    ...
    the upheld blocks and nothing else, and nothing can go missing between the decision
    and the outreach."""
    o = R.grade(ledger)
...
def test_a_denied_escalation_is_stranded_and_counted(ledger):
    ...
    and the case does not get handled more cheaply, it goes unhandled. That is a
