...
def test_replay_mode_refuses_to_invent(tmp_path, store, model):
    """The demo dry run must fail loudly on a cache miss instead of falling
    back to a template."""
...
def test_replay_does_not_depend_on_local_credentials(tmp_path):
    """The cache is committed so the demo runs off-network anywhere.

    The key hashes the provider in, and the provider is auto-detected from
    whichever API key happens to be present. A clone with no key detected a
    different provider, hashed a different key, missed all fourteen recorded
    replies and fell back to templates. Nothing failed and nothing warned, so
    everyone but the author saw a demo with no model output in it while the
    README said the opposite.
    """
    from agent.llm import LLMClient, GEMINI_MODEL

    system, messages = "case note system prompt", [{"role": "user", "content": "one case"}]
    recorded = LLMClient("offline", cache_dir=str(tmp_path),
                         provider="gemini", model=GEMINI_MODEL)
    key = recorded.cache_key(system, messages, None, max_tokens=2000,
                             effort="medium")
    recorded._write_cache(key, {"text": "the recorded note", "usage": {},
                                "model": GEMINI_MODEL, "provider": "gemini"})

    # same machine, no Gemini key: detect_provider() lands somewhere else
    clone = LLMClient("offline", cache_dir=str(tmp_path), provider="anthropic")
    assert clone.provider != "gemini"
    got = clone.complete(system, messages, template=lambda: "TEMPLATE")
    assert got.source == "cache", "replay fell back to a template with no key present"
    assert got.text == "the recorded note"
