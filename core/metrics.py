...
class Deployment:
    ...
    rather than measured, and it errs pessimistic. If honest and
...
def baseline_human_rationed(ledger: Ledger, vault, coverage: float,
    ...
    reach. Review cost is charged for reviewed cases only, since that is what
    rationing is for.
    """
    pids = ledger.payment_ids()
    y_bad = vault.labels(pids)
