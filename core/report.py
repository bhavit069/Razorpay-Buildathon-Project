...
# --- the report -------------------------------------------------------------
def build(data_dir="data300k", artifacts="artifacts", out=os.path.join("docs", "METRICS.md")) -> str:
    ...
    A(f"Those rows together are the actual claim. A reviewer beats this system "
      ...
      f"{cr(o.net_contribution_inr)}. The gap is coverage, not judgment. This "
      ...
      f"never gets to. Those are the small ones where the customer is least "
      f"likely to complain and most likely to just leave.")
    A("")
    ...
    if sc is not None:
        ...
        A(f"Each seed is graded twice. The **shipped** columns use cap "
          ...
          f"policies, and an earlier draft of this section did just that.")
        A("")
        ...
        A(f"**Precision is not the stable quantity it looked like.** At the shipped cap "
          ...
          f"the threshold on each world and therefore absorbs the very variation "
