...
const { B, trace, verdictHTML, decide, vectorise, rawScore, calibrate,
        ...
        newRun, liveTick, queueAt, queueRows, coverageBars } =
  Object.fromEntries(["B","trace","verdictHTML","decide","vectorise","rawScore",
    "calibrate","WRITTEN","FIELDS","fieldHTML","TONE","rPlan","pReturn",
    "crossover","CH","newRun","liveTick","queueAt","queueRows",
    "coverageBars"].map(n => [n, get(n)]));
...
check("no rung the ladder CHOSE is sent at a loss", (() => {
  ...
  // or not the arithmetic likes it. That is what escalating means. Those
...
check("the live board's books balance", (() => {
  ...
  // because those are trimmed. That is how the mix panel ended up
...
/* ---- the cap slider ------------------------------------------------------
   Dragging the cap repaints the deployment panel and the coverage chart. Those
   numbers are not carried per cap in the bundle; they are derived from the
   frontier row. So the derivation gets checked against the Python queue
   arithmetic, or the slider is only animating a guess. */
check("the derived queue arithmetic reproduces Python at the shipped cap", (() => {
  const bad = [];
  const r = B.frontier.find(x => x.cap === B.cap);
  if (!r) return ["no frontier row at the shipped cap"];
  const q = queueAt(r, B.margin), t = B.queue.terms;
  if (Math.abs(q.drag - t.drag_inr) > 0.01)
    bad.push(`drag ${q.drag.toFixed(2)} against Python ${t.drag_inr.toFixed(2)}`);
  if (Math.abs(q.breakeven - B.queue.breakeven) > 1e-9)
    bad.push(`breakeven ${q.breakeven} against Python ${B.queue.breakeven}`);
  for (const row of B.queue.rows){
    const got = q.at(row.rate);
    if (Math.abs(got - row.net_contribution_inr) > 0.01)
      bad.push(`rate ${row.rate}: ${got.toFixed(2)} against ${row.net_contribution_inr.toFixed(2)}`);
  }
  return bad;
})());

check("the deployment panel stays consistent at every cap on the slider", (() => {
  const bad = [];
  for (const r of B.frontier){
    const q = queueAt(r, B.margin);
    if (Math.abs(q.at(1) - r.contribution) > 0.01)
      bad.push(`cap ${r.cap}: inline row ${q.at(1).toFixed(2)} but the frontier says ${r.contribution.toFixed(2)}`);
    if (isFinite(q.breakeven) && Math.abs(q.at(q.breakeven)) > 0.01)
      bad.push(`cap ${r.cap}: net at breakeven is ${q.at(q.breakeven).toFixed(2)}, should be 0`);
    if (!queueRows(r).includes("<tr")) bad.push(`cap ${r.cap}: no rows rendered`);
  }
  return bad;
})());

check("only the system bar moves with the cap, and it moves to the right number", (() => {
  /* The label and the bar are two different things. An earlier version of this
     check only looked for the percentage in the text, and a mutation that
     pinned the BAR to the shipped recall while the label kept tracking the cap
     sailed straight through it. Assert the geometry. */
  const bad = [];
  const widths = h => (h.match(/width:([\d.]+)%/g) || []).map(x => +x.slice(6, -1));
  const hr = B.human.rationed;
  const fixed = [...Object.keys(hr).sort((a, b) => +a - +b).map(k => hr[k].recall),
                 B.human.full.recall];
  const base = widths(coverageBars(B.frontier[0]));
  for (const r of B.frontier){
    const w = widths(coverageBars(r));
    if (w.length !== fixed.length + 1){
      bad.push(`cap ${r.cap}: ${w.length} bars, expected ${fixed.length + 1}`); continue; }
    for (let i = 0; i < w.length - 1; i++)
      if (Math.abs(w[i] - base[i]) > 0.05)
        bad.push(`cap ${r.cap}: reviewer bar ${i} moved, ${base[i]} -> ${w[i]}`);
    // barList scales every bar against the largest value in the list
    const max = Math.max(...fixed, r.recall);
    const want = 100 * r.recall / max;
    if (Math.abs(w[w.length - 1] - want) > 0.05)
      bad.push(`cap ${r.cap}: system bar is ${w[w.length - 1].toFixed(1)}% wide, ` +
               `recall ${(100 * r.recall).toFixed(1)}% wants ${want.toFixed(1)}%`);
    if (!coverageBars(r).includes((100 * r.recall).toFixed(1) + "%"))
      bad.push(`cap ${r.cap}: system bar label does not read ${(100 * r.recall).toFixed(1)}%`);
  }
  return bad;
})());
