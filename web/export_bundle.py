...
from web.typical import typical_values
...
def recovery_block(ledger, vault, n_check: int = 400) -> dict:
    ...
    cases, so web/check_agent.js can prove the JavaScript reproduces it
...
def build(data_dir="data300k", out=OUT):
    ...
    with open(os.path.join(data_dir, "stats.json"), encoding="utf-8") as fh:
        stats = json.load(fh)
    # Both of these ship with the repo. Say how to get them back rather than
    # letting a bare FileNotFoundError be the whole message.
    def _need(name, how):
        path = os.path.join("artifacts", name)
        if not os.path.exists(path):
            raise FileNotFoundError(
                f"artifacts/{name} is missing. It ships with the repo; "
                f"regenerate it with `{how}`. The console quotes it, so it is "
                f"not optional.")
        with open(path, encoding="utf-8") as fh:
            return json.load(fh)

    headline = _need("headline.json", "python run.py metrics")
    seeds = _need("seed_check.json", "python run.py seeds")

    ledger = backtest_run(store, model, cfg, "holdout")
    o = grade(ledger, vault, stepup)

    # web/check_dashboard.js replays the page's own functions against these.
    # Written here because this is where the same numbers already exist. It
    # used to be a file on disk that nothing regenerated, so the check that
    # reads it could not run on a fresh clone and was never wired into the
    # dry run at all.
    _by_pid = {r.payment_id: r for r in ledger.records}
    _rows = []
    for _c, _row, _p in zip(ho, X, want):
        _r = _by_pid.get(_c.payment_id)
        if _r is None:
            continue
        _rows.append({"payment_id": _c.payment_id,
                      "x": [float(v) for v in _row],
                      "p_bad": float(_p),
                      "action": _r.action,
                      "amount_inr": float(_r.amount_inr),
                      "orders": float(_r.network_orders_prior),
                      "tenure": float(_r.network_tenure_days)})
    with open(os.path.join("artifacts", "check_cases.json"), "w",
              encoding="utf-8") as fh:
        json.dump(_rows, fh, separators=(",", ":"))
