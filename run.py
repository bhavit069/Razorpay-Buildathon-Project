#!/usr/bin/env python3
"""
Task runner. `make` isn't installed on this box, so this is the canonical entry
point; the Makefile just delegates here for parity with the architecture doc.

    python run.py data        # 100k baseline (reproduces the committed stats)
    python run.py data300k    # 300k working set (tighter CIs, Day-1 gate)
    python run.py validate    # is the signal there, and does the network add lift?
    python run.py moat        # the same question, priced in rupees
    python run.py sweep       # intercept sensitivity across block-rate regimes
    python run.py metrics     # regenerate METRICS.md + artifacts/ figures
    python run.py test        # the Phase 1 test suite
    python run.py notebooks   # rebuild and re-execute the explainers
    python run.py seeds       # how much does all of this move between worlds?
    python run.py agent       # Phase 2: run the agent over the holdout docket
    python run.py docs        # refill generated blocks in IDEA.md and DATA_CARD.md
    python run.py room        # build the case room, one HTML file, open it in a browser
    python run.py console     # build the five-page browser console
    python run.py serve       # build it and serve on http://localhost:4000
    python run.py warm        # record real model replies for the demo cases
    python run.py recovery    # regenerate RECOVERY.md
    python run.py dry         # pre-demo dry run, network cut
    python run.py pr          # package the deliverable into ../pr
    python run.py clean
"""
import os
import shutil
import subprocess
import sys

ROOT = os.path.dirname(os.path.abspath(__file__))
PY = sys.executable
# The pipeline prints rupee symbols; Windows consoles default to cp1252.
ENV = {**os.environ, "PYTHONUTF8": "1", "PYTHONPATH": ROOT}


def sh(*args):
    print(f"$ {' '.join(str(a) for a in args)}", flush=True)
    r = subprocess.run([PY, *[str(a) for a in args]], cwd=ROOT, env=ENV)
    if r.returncode:
        sys.exit(r.returncode)


def data():
    sh("simulation/generate.py", "--n", 100_000, "--seed", 42, "--out", "./data")


def data300k():
    # --customers scales with --n by default; holding it fixed would triple
    # orders-per-customer and push the block rate from 2.8% to 6.3%.
    sh("simulation/generate.py", "--n", 300_000, "--seed", 42, "--out", "./data300k")


def validate():
    sh("simulation/validate_signal.py", _dataset())


def moat():
    sh("simulation/moat_ledger.py", _dataset())


def sweep():
    for i in ("-3.5", "-4.4", "-4.9", "-5.4"):
        out = f"./data/_sweep/i{i}"
        print(f"\n===== intercept {i} =====", flush=True)
        sh("simulation/generate.py", "--n", 100_000, "--seed", 42,
           "--intercept", i, "--out", out)
        sh("simulation/validate_signal.py", out)


def metrics():
    sh("-m", "core.report")


def test():
    sh("-m", "pytest", "tests", "-q")


def notebooks():
    """Rebuild and re-execute the five explainers."""
    b = os.path.join(ROOT, "docs", "notebooks", "build_notebooks.py")
    if not os.path.exists(b):
        sys.exit("The notebook builder is not in this copy. docs/notebooks/ holds "
                 "the built notebooks with their output already in them, so open "
                 "those as they are; rebuilding needs the working tree.")
    sh("docs/notebooks/build_notebooks.py")


def seeds():
    sh("tools/seed_check.py")


def agent():
    sh("tools/demo.py", "--limit", 100)


def dry():
    """Everything that happens on stage, with the network cut."""
    sh("tools/dry_run.py")


def docs():
    """Refill the generated blocks in IDEA.md and DATA_CARD.md."""
    sh("-m", "core.docs")


def room():
    """The one screen. Writes a self-contained HTML file, no server."""
    sh("-m", "web.case_room")


def serve():
    """Build the pages and serve them on http://localhost:4000."""
    sh("-m", "web.serve")


def console():
    """The browser console: five pages, one self-contained HTML file. The agent
    page runs the exported model, so the exporter verifies it against Python
    before writing anything."""
    sh("-m", "web.dashboard")


def recovery():
    """RECOVERY.md: the outreach ladder, its crossovers and its assumptions.
    Separate from METRICS.md on purpose - that file is frozen and measured,
    this one rests on declared rates and says so in its first line."""
    sh("tools/make_recovery.py")


def warm():
    """Record real model replies for the demo cases into agent/cache/."""
    sh("tools/warm_cache.py")


def pr():
    """Package the deliverable into ../pr: one documentation file, the built
    pages under demo/, no HTML in artifacts, no build scripts."""
    sh("tools/make_pr.py")


def clean():
    for d in ("data", "data300k", "artifacts"):
        p = os.path.join(ROOT, d)
        if os.path.isdir(p):
            shutil.rmtree(p)
            print(f"removed {d}/")


def _dataset():
    """Prefer the 300k working set; fall back to the 100k baseline."""
    for d in ("./data300k", "./data"):
        if os.path.isdir(os.path.join(ROOT, d.lstrip("./"))):
            return d
    sys.exit("No dataset found. Run `python run.py data300k` first.")


TASKS = {f.__name__: f for f in (data, data300k, validate, moat, sweep, metrics,
                                 test, notebooks, seeds, agent, docs, room, console, serve, warm,
                                 dry, recovery, pr, clean)}

if __name__ == "__main__":
    name = sys.argv[1] if len(sys.argv) > 1 else ""
    if name not in TASKS:
        sys.exit(f"usage: python run.py {{{'|'.join(TASKS)}}}")
    TASKS[name]()
