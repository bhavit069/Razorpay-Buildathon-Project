...
| Surfaces | case room, 7-page console on :4000, **5-scene stage on :4005** |
| METRICS.md | frozen 2026-08-30, regenerates from seed |
| Test suite | **117 passed**, 0 failed, 0 skipped |
| Pre-demo dry run, network cut | **18 of 18** |
...
python run.py stage       # the room version, :4005 (run it alongside serve)
...
### The stage — http://localhost:4005

Added 4 September, alongside the console rather than instead of it. Same
bundle, same engine file, different job: the console is for someone sitting
down with it, the stage is for someone walking past the laptop.

One screen, no navigating, five scenes on the number keys **1**–**5** or the
arrow keys. Dark by default with a light toggle, remembered per browser.

| | |
|---|---|
| **1 Live** | the pipeline with real orders moving through it, totals running |
| **2 Why** | the problem in three numbers and one bar |
| **3 One case** | evidence, decision and recovery ladder for a single order |
| **4 Ladder** | the five channels and how severity falls out of the arithmetic |
| **5 Honest** | the four numbers that moved against us |

Both servers run side by side and cross-link — `:4000/stage` and
`:4005/console` reach each other, `/case` is the case room on either.

**There is one copy of the model and one of the recovery ladder**, in
`service/engine.js`, injected into both pages at build time. That is the whole
reason the refactor happened: two transliterated copies of a model is precisely
the bug this project keeps catching elsewhere. `check_stage.js` asserts it
directly — it loads both built pages and compares their live configuration
objects and their answers on every case.
...
### 7.7 The stage, built 4 September
...
**B25. A bundle map indexed by a stringified float.** `h.rationed["0.1"]` — the
key is `"0.10"` — returned undefined, `pct()` rendered it **0.0%**, and every
leak check passed, because zero is a perfectly good number. The human-baseline
bar chart on the stage silently showed the 10%-coverage reviewer recovering
nothing. Fixed by iterating the map instead of guessing at its keys, and two
checks added for the class rather than the instance: no panel may render an
exact `0.0%`, and no bundle map may be indexed by a hand-written key.

**B26. Three headline figures were typed into the template.** `Rs 150`,
`Rs 0.90–1.86 cr` and `Rs 2,11,715` were literals that happened to be correct
and would have stayed correct right up until the generator was reseeded, at
which point the page would have lied quietly. All three now read from the
bundle. The check scans the template source rather than the output, because a
hardcoded number renders identically to a read one.

**T2, another test bug.** The stage check anchored on `stageHtml.indexOf(
"function liveTick")` — which still matches after a rename to `liveTickX`,
because it is a prefix — and sliced up to the *start* of the coin flip, so a
mutant that multiplied the draw by the answer key sat just past the window and
read as a pass. Both were found by mutation testing the check itself, which is
the only way that class of thing surfaces.
