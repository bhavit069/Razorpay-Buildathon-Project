...
class LLMClient:
    ...
    def _recorded_pairs(self) -> list:
        """(provider, model) combinations the cache was actually recorded with.

        The key hashes the provider in, and the provider is auto-detected from
        whichever API key happens to be present. A clone with no key detects a
        different provider, hashes a different key and misses every recorded
        reply, so the demo silently falls back to templates on any machine but
        the one that recorded it. Replay must not depend on local credentials.
        """
        ...
        for n in names:
            if not n.endswith(".json"):
                continue
            try:
                with open(os.path.join(self.cache_dir, n), encoding="utf-8") as fh:
                    r = json.load(fh)
            except Exception:
                continue
            pair = (r.get("provider"), r.get("model"))
    ...
    def _lookup(self, system, messages, schema, **kw):
        """Exact key first, then the pairs the cache was recorded under."""
        hit = self._read_cache(self.cache_key(system, messages, schema, **kw))
        if hit is not None:
            return hit
        for prov, model in self._recorded_pairs():
            if (prov, model) == (self.provider, self.model):
                continue
            k = self._cache_key_for(prov, model, system, messages, schema, **kw)
            hit = self._read_cache(k)
            if hit is not None:
                return hit
        return None

    def _cache_key_for(self, provider, model, system, messages, schema=None, **kw) -> str:
        blob = json.dumps(
    ...
    def complete(self, system: str, messages: list, *, schema=None,
        ...
        hit = self._lookup(system, messages, schema, max_tokens=max_tokens,
                           effort=effort)
        if hit is not None:
