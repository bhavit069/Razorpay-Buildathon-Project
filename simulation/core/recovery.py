...
the contact cost and book nothing, which is why a cheap channel wins on small
orders and an expensive one has to earn its place.
...
class Channel:
    ...
    two_way can it carry a question and take an answer? A one-way nudge cannot
            run a verification exchange, which is what STEP_UP needs, and that
            constraint - not persuasiveness - is why a voice call exists here
    """
    key: str
    name: str
    cost_inr: float
...
CHANNELS = (
    ...
    Channel("voice", "Agentic voice call", 14.00, 18.0, True, 0.35, 0.88,
            ...
            "well because the agent can finish the order on the line. It is not a "
            "better nudge than an SMS. It is the cheapest channel that can ask a "
            "question and hear the answer, which is the only reason it is here."),
...
class RecoveryConfig:
    ...
    half_life_min
        ...
        together, which is the correct way for two documents to depend on each
        other.
    ...
    rung_correlation
        ...
        the first is shrunk by this. 0 would be full independence, which is the
        optimistic lie. Declared at 0.45, which is the pessimistic direction.

    count_ltv / churn_on_false_decline / ltv_horizon_days
        A wrongly refused customer may not just skip this order. The platform
        can see their whole history, so it can price the relationship, which is
        the one thing a single merchant cannot do. Off by default: it makes
...
def plan(action: Action | str, ev_release_inr: float, amount_inr: float,
    ...
    Greedy by expected value, and the greed is the point: each rung is added
    ...
    # escalations on the floor, which is the worst possible thing to do with a
    # case whose whole point is that nobody has looked at it yet.
    #
...
class RecoveryOutcome:
    ...
    # the case is not merely handled more cheaply, it is not handled: no rung
