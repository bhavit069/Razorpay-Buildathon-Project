...
.wrap{display:grid;grid-template-columns:290px 1fr;min-height:calc(100vh - 61px)}
.list{border-right:1px solid var(--line);overflow-y:auto;max-height:calc(100vh - 61px);
background:var(--card)}
...
main{padding:24px 28px;overflow-y:auto;max-height:calc(100vh - 61px)}
...
padding-top:12px}
@media(max-width:860px){.wrap{grid-template-columns:1fr}.cols{grid-template-columns:1fr}
...
function show(i){
 ...
 h+=`<div class="verdict">${c.verdict}<div class="src">verdict source: ${c.verdict_source}</div></div>`;
...
def build(out: str = OUT, data_dir: str = "data300k", limit: int = 400) -> str:
    ...
    listing = "".join(
        ...
        f'<span class="tag {c["tone"]}">{c["action"]}</span></div>'
...
{n_role} tagged as demo cases and listed first. Generated by
<code>python -m service.case_room</code>.</span></header>
