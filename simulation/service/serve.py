...
"""Serve the console and the case room on localhost.
    ...
    python -m service.serve            # http://localhost:4000
    python -m service.serve --port 8080
    python -m service.serve --no-build # serve what is already in artifacts/
...
class Handler(http.server.SimpleHTTPRequestHandler):
    ...
    def __init__(self, *a, **kw):
        super().__init__(*a, directory=ARTIFACTS, **kw)

    def translate_path(self, path):
        clean = path.split("?", 1)[0].split("#", 1)[0].rstrip("/") or "/"
        if clean in ROUTES:
            return os.path.join(ARTIFACTS, ROUTES[clean])
        # the served directory already is artifacts/, so an /artifacts/ prefix
        # would look for artifacts/artifacts/. Accept it anyway, because that is
        # the path people type after reading the startup banner.
        if clean.startswith("/artifacts/"):
            path = path[len("/artifacts"):]
        elif clean == "/artifacts":
            path = "/"
