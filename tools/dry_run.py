...
step that reaches for an API fails here instead of working on this
...
# runnable directly, not only through run.py
...
def main():
    ...
    def doc_with(*names):
        """Return (name, text) for the first of these that exists under docs/.

        The working tree keeps the generated documents separate; the packaged
        version folds them into one DOCUMENTATION.md. The numbers have to be
        checked either way."""
        for n in names:
            p = os.path.join(ROOT, "docs", n)
            if os.path.exists(p):
                return n, io.open(p, encoding="utf-8").read()
        return None, None
    ...
    def data_present():
        need = ["data300k/appeal_queue.csv", "data300k/stats.json",
                ...
                "demo/case_room.html", "artifacts/tables.md"]
        missing = [p for p in need if not os.path.exists(os.path.join(ROOT, p))]
        name, _ = doc_with("METRICS.md", "DOCUMENTATION.md")
        if name is None:
            missing.append("docs/METRICS.md or docs/DOCUMENTATION.md")
        if missing:
            raise FileNotFoundError(f"missing: {', '.join(missing)}")
        return f"{len(need) + 1} required files present, measured numbers in {name}"
    ...
    def metrics_match_headline():
        ...
        name, md = doc_with("METRICS.md", "DOCUMENTATION.md")
        if md is None:
            raise FileNotFoundError("no METRICS.md and no DOCUMENTATION.md under docs/")
        want = {f"{h['recall']:.1%}", f"{h['precision']:.1%}",
                f"{h['abstention_rate']:.1%}"}
        missing = [w for w in want if w not in md]
        if missing:
            raise AssertionError(f"{name} does not contain {missing}")
        return (f"cap {h['cap']}, recall {h['recall']:.1%}, "
                f"precision {h['precision']:.1%}, checked against {name}")
    ...
    # --- the screen ----------------------------------------------------------
    def build_room():
        from web.case_room import build
        p = build(out="demo/case_room.html", data_dir="data300k", limit=400)
    ...
    def no_external_refs():
        html = io.open(os.path.join(ROOT, "demo/case_room.html"), encoding="utf-8").read()
    ...
    def build_console():
        ...
        from web.dashboard import build as build_console_page
        p = build_console_page(out="demo/console.html", data_dir="data300k")
        html = io.open(os.path.join(ROOT, p), encoding="utf-8").read()
    ...
    def browser_model():
        """The page ships a transliterated copy of the model, so the only claim
        worth checking is that it answers the same as Python. This runs the
        page's own functions over the whole holdout, not a sample. It existed
        and passed for a week without being wired into anything, which is its
        own small lesson about checks nobody runs."""
        r = subprocess.run(["node", "web/check_dashboard.js"], cwd=ROOT,
                           capture_output=True, text=True,
                           stdin=subprocess.DEVNULL)
        if r.returncode:
            raise AssertionError(r.stdout.strip()[-500:])
        out = r.stdout.strip().splitlines()
        return " | ".join(x.split(":", 1)[1].strip() for x in out
                          if x.startswith(("model", "policy")))
    try:
        c("the browser runs the same model as Python", browser_model)
    except FileNotFoundError:
        print("  [skip] the browser runs the same model as Python (no node on PATH)")
    ...
    def responsive():
        ...
        r = subprocess.run(["node", "web/check_responsive.js"], cwd=ROOT,
                           capture_output=True, text=True,
                           stdin=subprocess.DEVNULL)
        if r.returncode:
    ...
    def agent_page():
        ...
        r = subprocess.run(["node", "web/check_agent.js"], cwd=ROOT,
                           capture_output=True, text=True,
                           stdin=subprocess.DEVNULL)
        if r.returncode:
            raise AssertionError(r.stdout.strip()[-500:])
    ...
    def recovery_doc():
        ...
        name, have = doc_with("RECOVERY.md", "DOCUMENTATION.md")
        if have is None:
            raise AssertionError("no RECOVERY.md and no DOCUMENTATION.md under "
                                 "docs/; run `python run.py recovery`")
        from core.backtest import run as backtest_run
        ...
        if name == "RECOVERY.md":
            ...
            how = "regenerates identically"
        else:
            # folded into the single document: every line of the generated body
            # still has to be in there, or the two have drifted apart
            gone = [ln for ln in want.strip().splitlines()
                    if ln.strip() and ln not in have]
            if gone:
                raise AssertionError(
                    f"{len(gone)} generated recovery line(s) missing from {name}, "
                    f"first: {gone[0][:70]!r}; run `python run.py docs`")
            how = f"every generated line present in {name}"
        n = sum(1 for line in want.splitlines() if line.startswith("## "))
        return f"{len(want) // 1024} KB, {n} sections, {how}"
